...
def fetch_data_for_period(py_trends, start_date, end_date, country_code, keyword, output_path):
    ...
    while attempts < 5:
        try:
            ...
            # attempts += 1
            time.sleep((2 ** attempts) * random.randint(60, 120))  # Exponential backoff with randomization
            attempts += 1
...
base_dir = r'C:\Users\Vikash\OneDrive\Desktop\gtrandes\finaldata'
###############################CHANGE HERE DIR NAME AS PER COUNTRY NAME ###########
data_dir = Path(f"{base_dir}/data/Switzerland")                                #
output_dir = Path(f"{base_dir}/Output/Switzerland")                            #
############# #########CHANGE HERE ################################################
countries = {'Switzerland': 'CH'}                                     #
keywords = [ 'Sustainability', 'UNFCCC', 'United Nations Framework Convention on Climate Change']                 #
##########################################################################
...
for keyword in keywords:
    ...
    ##############################   CHNAGE HERE COUNTRY    ###################################################################################
        fetch_data_for_period(py_trends, current_date, next_date - relativedelta(days=1), countries['Switzerland'], keyword, keyword_data_path)
